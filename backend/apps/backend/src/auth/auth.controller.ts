...
export class AuthController {
  ...
  async updateProfile(@Req() req: AuthenticatedRequest, @Body() body: { name?: string }, @UploadedFile() file?: Express.Multer.File) {
    const data: any = {};
    if (body.name) data.name = body.name;
    if (file) {
      const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1e9);
      const filename = `avatar-${uniqueSuffix}-${file.originalname.replace(/[^a-zA-Z0-9.]/g, '')}`;
      await this.storageService.uploadBuffer(file.buffer, filename, file.mimetype);
